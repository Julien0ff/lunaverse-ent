...
export default function MessagesPage() {
  ...
  const loadMessages = async (friendId: string, silent = false) => {
    ...
    if (res.ok) {
      const data = await res.json()
      // Only set if different to avoid re-renders (basic check by length but ideally check IDs)
      setMessages(prev => {
  ...
  const sendMessage = async (e?: React.FormEvent, directContent?: string) => {
    ...
    // Mark current length to handle optimistic rollback if needed
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
              ...
              {sentRequests.length > 0 && (
                <div className="opacity-80">
                  <h3 className="text-xs font-black text-discord-muted uppercase tracking-widest mb-2 px-1 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5" /> Envoyées ({sentRequests.length})
                  </h3>
                  <div className="space-y-2">
                    {sentRequests.map(r => (
                      <div key={r.id} className="flex items-center justify-between p-2.5 bg-white/5 border border-white/10 rounded-xl opacity-60">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="text-xs font-bold text-white truncate">{r.username}</span>
                        </div>
                        <button onClick={() => handleFriendAction('remove', r.friend_id)} className="w-6 h-6 flex items-center justify-center hover:bg-discord-error/20 text-discord-error rounded-md transition-colors"><X className="w-3 h-3 flex-shrink-0" /></button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="space-y-1 mt-2">
                  ...
                  {acceptedFriends.length === 0 && <p className="text-xs text-discord-muted p-2">Aucun ami pour le moment.</p>}
                </div>
              </div>
